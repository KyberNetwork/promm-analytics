...
const Search = ({ ...rest }: React.HTMLAttributes<HTMLDivElement>) => {
  ...
  const savedTokenAddresses: string[] = []
  Object.keys(savedTokens[activeNetwork.chainId] ?? {}).forEach((address: string) => {
    savedTokenAddresses.push(address)
  })

  const savedPoolAddresses: string[] = []
  Object.keys(savedPools[activeNetwork.chainId] ?? {}).forEach((address: string) => {
    savedPoolAddresses.push(address)
  })

  // get date for watchlist
  const watchListTokenData = useTokenDatas(savedTokenAddresses)
  const watchListPoolData = usePoolDatas(savedPoolAddresses)
              ...
              .map((t, i) => {
                return (
                  <HoverRowLink onClick={() => handleNav(networkPrefix(activeNetwork) + 'tokens/' + t.address)} key={i}>
                    <ResponsiveGrid>
                      <RowFixed>
                        <CurrencyLogo address={t.address} activeNetwork={activeNetwork} />
                        <TYPE.label ml="10px">
                        ...
                        <SavedIcon
                          ...
                          fill={savedTokenAddresses.includes(t.address)}
                          onClick={(e) => {
                            e.stopPropagation()
                            addSavedToken(activeNetwork.chainId, t)
                          }}
                        />
                      </RowFixed>
                      <HideSmall>
                    ...
                    <ResponsiveGrid key={i}>
                      <RowFixed>
                        ...
                        <SavedIcon
                          ...
                          fill={savedPoolAddresses.includes(p.address)}
                          onClick={(e) => {
                            e.stopPropagation()
                            addSavedPool(activeNetwork.chainId, p)
                          }}
                        />
                      </RowFixed>
                      <HideSmall>
